...
int main (int argc, char *argv[])
{
  ...
  if ((id==0)&&  OUTPUT)
	...
	MPI_Barrier(MPI_COMM_WORLD);
	if ((id==0)&&  OUTPUT)
	  printf("* Calculating the MI value.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	...
	double *mitemp=calcMI(llll,number,length);
	k=0;
	for (i=0;i<length;i++)
	  for (j=0;j<length;j++)
	  {
  ...
  MPI_Barrier(MPI_COMM_WORLD);
	if ((id==0)&&  OUTPUT)
	  printf("* MI calculation finished.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	if ((id==0)&&  OUTPUT)
	  printf("* Saving MI result.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	if (id==0)
	{
		if (f=fopen("MIsave.save","wb"))
		{
  ...
  MPI_Bcast(&save,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	...
	for (i=id;i<length;i+=size)
	{
	  int j,k;
		...
		int *tempp;
		tempp=calcMI_P(llll,number,length,i,times,save);
		j=0;
		while (j<length)
		{
		  p[i][j]=tempp[j];
