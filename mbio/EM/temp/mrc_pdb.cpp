...
bool CompareAtom (ATOM i,ATOM j) { return (i.id<j.id); }

int GetAtomID(const char *name)
{
	if(strcmp(name,"H")==0)  return 0;
	if(strcmp(name,"HE")==0) return 1;
	if(strcmp(name,"LI")==0) return 2;
	if(strcmp(name,"BE")==0) return 3;
	if(strcmp(name,"B")==0)  return 4;
	if(strcmp(name,"C")==0)  return 5;
	if(strcmp(name,"N")==0)  return 6;
	if(strcmp(name,"O")==0)  return 7;
	if(strcmp(name,"F")==0)  return 8;
	if(strcmp(name,"Ne")==0) return 9;
	if(strcmp(name,"Na")==0) return 10;
	if(strcmp(name,"MG")==0) return 11;
	if(strcmp(name,"AL")==0) return 12;
	if(strcmp(name,"SI")==0) return 13;
	if(strcmp(name,"P")==0)  return 14;
	if(strcmp(name,"S")==0)  return 15;
	if(strcmp(name,"CL")==0) return 16;
	if(strcmp(name,"AR")==0) return 17;
	if(strcmp(name,"K")==0)  return 18;
	if(strcmp(name,"CA")==0) return 19;
	if(strcmp(name,"FE")==0) return 25;

	//to be continued
	return 0;
}

CMPLXf GetStructureFactor(vector<ATOM> &atoms, float h, float k, float l)
{
	...
	{
		if(atoms[i].id!=curAtomID) 
		{
			curAtomID=atoms[i].id;
		...
		amp=exp(-r2*atoms[i].bf/2.0)*atoms[i].occ*AtomDiffFactor;
		phase=-2.0*PI*(h*atoms[i].x+k*atoms[i].y+l*atoms[i].z);
...
int readPDB(const char* filename,vector<ATOM> &atoms, float sx, float sy, float sz)
{
	...
	ATOM atom;
	char buffer[512],tmp[32];

	while(!feof(fp)){
		if(!fgets(buffer,512,fp)) continue;

		if(buffer[0]!='A' || buffer[1]!='T' || buffer[2]!='O' || buffer[3]!='M') continue;

		if(sscanf(buffer,"ATOM%*2c%*5c%*1c%4c%*c%*3c%*c%*c%*4d%*c%*3c%8c%8c%8c%6c%6c%*10c%*2c%*2c",name,x,y,z,occ,bf)<0) continue;
...
// void fft3d(float* buf, int nsam)
// {
// 	fftwf_plan plan_fft=fftwf_plan_dft_r2c_3d(nsam,nsam,nsam,buf,reinterpret_cast<fftwf_complex *>(buf),FFTW_ESTIMATE);  
// 	fftwf_execute(plan_fft);
// 	fftwf_destroy_plan(plan_fft);
// }

void ifft3d(float* buf, int nsam, float* out)
{
	fftwf_plan plan_fft=fftwf_plan_dft_c2r_3d(nsam,nsam,nsam,reinterpret_cast<fftwf_complex *>(buf),out,FFTW_ESTIMATE);  
	fftwf_execute(plan_fft);
	fftwf_destroy_plan(plan_fft);
}

//Input1: 1model.pdb 2output.mrc 3Box 4PixelSize(A) 5Resolution(A)
//Input2: 1model.pdb 2output.mrc 3Box 4PixelSize(A) 5Resolution(A) 6ShiftX(A)  7ShiftY(A)  8ShiftZ(A)
int main(int narg, char* argc[])
{
	if(narg==1)
	{
		printf("Convert pdb file to mrc map.\n\n");
		printf("  Input1: model.pdb output.mrc Box PixelSize(A) Resolution(A)\n");
		printf("  Input2: model.pdb output.mrc Box PixelSize(A) Resolution(A) ShiftX(A) ShiftY(A) ShiftY(A)\n");
		printf("          *Note: the shift will be ADDED to all coordinates.\n");
		printf("\nWrote by Xueming @ ucsf Cheng lab\n");
		printf("Optimized by Wenzhi Mao @ Tsinghua University Haipeng Gong Lab\n");
		return 0;
	}
	...
	int nsam=atoi(argc[3]);
	float psize=atof(argc[4]);
	float res=atof(argc[5]);
	float sx=0.0;
	float sy=0.0;
	float sz=0.0;
	...
	vector<ATOM> atoms;
	if(readPDB(argc[1], atoms,sx,sy,sz)<=0)
	{
		printf("Abort.\n");
		return 1;
	}
	sort(atoms.begin(),atoms.end(),CompareAtom);

	int i,j,k,ii,jj,kk,id,id0,id1;
	int isodd=nsam%2;
	int hnsaml=(nsam/2+1);
	int nsaml=hnsaml*2;
	int hnsam=nsam/2;
	...
	{
		rmax=hnsam;
		printf("Warning: resolution is larger than Nyquist. \n");
	}

	float *pfft=new float[nsam*nsam*nsaml];
	float *buf=new float[nsam*nsam*nsam];
	CMPLXf *pfftc=(CMPLXf *)pfft;

	int step=int(nsam/10.0+0.5),count=0,p=0;

	#pragma omp parallel for schedule(dynamic) firstprivate(isodd,hnsaml,hnsam,ii,nsam,id0,j,jj,id1,k,kk,id,rmax,Fpsize,atoms) shared(count,p)
	for(i=-hnsam;i<hnsam+isodd;i++)
	{
		...
		for(j=-hnsam;j<hnsam+isodd;j++)
		{
			...
			for(k=0;k<hnsaml;k++)
			{
				...
				else pfftc[id]=GetStructureFactor(atoms, k*Fpsize, j*Fpsize, i*Fpsize);
			}
		}
		#pragma omp critical
		{
	...
	// 			{
	// 			}
	// 			else fprintf(fp,"%.6lf %.6lf\n",sqrt(pfftc[id].x*pfftc[id].x+pfftc[id].y*pfftc[id].y),sqrt(i*i+j*j+k*k));
	// 		}
	// 	}
	// }
	// fclose(fp);

	ifft3d(pfft,nsam,buf);
	...
	for(i=0;i<nsam;i++)
	{
		...
		for(j=0;j<nsam;j++)
		{
			...
			for(k=0;k<nsam;k++)
			{
				buf[id1+k]=buf[id1+k]/scale;
			}
		}
	}

	MRC mrc;
	if(mrc.open(argc[2],"wb")==0)
	{
		printf("Error: Failed to create mrc for output.\n");
		return 1;
	}
	mrc.createMRC(buf,nsam,nsam,nsam);
	mrc.m_header.mx=nsam;
	mrc.m_header.my=nsam;
	mrc.m_header.mz=nsam;
	mrc.m_header.cella[0]=nsam*psize;
	mrc.m_header.cella[1]=nsam*psize;
	mrc.m_header.cella[2]=nsam*psize;
	mrc.m_header.origin[0]=-sx;
	mrc.m_header.origin[1]=-sy;
	mrc.m_header.origin[2]=-sz;
	memccpy(mrc.m_header.map,"MAP ",'\0',4);
	mrc.updateHeader();
	mrc.close();
