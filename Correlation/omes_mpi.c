...
int main (int argc, char *argv[])
{
  ...
  if ((id==0)&&  OUTPUT)
	...
	MPI_Barrier(MPI_COMM_WORLD);
	if ((id==0)&&  OUTPUT)
	  printf("* Calculating the OMES value.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	...
	double *omtemp=calcOMES(llll,number,length);
	k=0;
	for (i=0;i<length;i++)
	  for (j=0;j<length;j++)
	  {
  ...
  MPI_Barrier(MPI_COMM_WORLD);
	if ((id==0)&&  OUTPUT)
	  printf("* OMES calculation finished.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	if ((id==0)&&  OUTPUT)
	  printf("* Saving OMES result.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	if (id==0)
	{
		if (f=fopen("OMESsave.save","wb"))
		{
     fwrite(&om[0][0],sizeof(double),length*length,f);
		}
		fclose(f);
	}
	if ((id==0)&&  OUTPUT)
	  printf("* OMES Saving finished.\n",id);
	MPI_Barrier(MPI_COMM_WORLD);
	double save=0;
	long all=length*length,done=0,cccc=0;
	if (id==0)
  {	
    double sortom[length*length];
	  k=0;
	  for (i=0;i<length;i++)
	  {
	    for (j=0;j<length;j++)
  ...
  MPI_Bcast(&save,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
	...
	for (i=id;i<length;i+=size)
	{
	  int j,k;
		...
		int *tempp;
		tempp=calcOMES_P(llll,number,length,i,times,save);
		j=0;
		while (j<length)
		{
		  p[i][j]=tempp[j];
